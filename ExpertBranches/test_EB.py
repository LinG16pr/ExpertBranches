...
def main():
    ...
    # After: missing, unexpected = net.load_state_dict(state, strict=False)
    ...
    # Optional: fail hard during debugging
    # assert len(missing) == 0 and len(unexpected) == 0, "Checkpoint didn't load strictly."
    net.eval()
    ...
    with torch.no_grad():
        ...
        with autocast():
            ...
            for imgs, masks, metas in pbar:
                ...
                # print("Unique mask:", torch.unique(masks).tolist())
                # print(IGNORE_VAL)
                # --------- CRITICAL: sanitize masks to prevent OOB ---------
                # Optional binarization (per-organ binary tasks)
                ...
                # print("Unique pred:", torch.unique(preds).tolist())
               

                # Safe CM update (ignores IGNORE_VAL)
                _safe_confusion_update(cm, preds, masks, args.num_classes, IGNORE_VAL)

                # Surface metrics with the same ignore
                ...
                masd_sum += float(batch_masd) if np.isfinite(batch_masd) else 0.0
